...
const B = "http://localhost:3402"; const results = [];
const rec = (id, status, title, ev = "") => { results.push({ group: "N-nextjs14", id, status, title, evidence: ev }); console.log(`${status.padEnd(17)} ${id.padEnd(7)} ${title}${ev ? " :: " + ev : ""}`); };
...
fs.writeFileSync("../results/nextjs14-http.json", JSON.stringify(results, null, 2));
